...
## 놓쳤던 디테일
 ...
 <p>파이썬에서 슬라이싱은 <code>range()</code>와 같이 [:2]를 한다면 2번 index전까지 리턴한다.</p>
 <p>이점을 활용하여 <code>sum(arr[:i+1])</code>코드를 이용하여 배열이 길이를 1씩 증가하는 누적합리스트를 리턴 받을 수 있다.</p>
